...
// Simple time formatter (can be replaced with date-fns if available)
function fToNow(date: string | number | Date | null): string {
  if (!date) return '';
  const now = new Date();
...
// Default assets directory - can be overridden via props if needed
    ...
    <ListItemAvatar>
      {notification.avatarUrl ? (
        ...
        >
          <Box
            component="img"
            src={`${DEFAULT_ASSETS_DIR}/icons/notification/${(notification.type === 'order' && 'ic-order') || (notification.type === 'chat' && 'ic-chat') || (notification.type === 'mail' && 'ic-mail') || (notification.type === 'delivery' && 'ic-delivery')}.svg`}
