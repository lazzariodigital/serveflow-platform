...
// Default assets directory - can be overridden by apps
 ...
 */
export function getFileIcon(input?: FileInput, assetsDir: string = DEFAULT_ASSETS_DIR): string {
  const format = detectFileFormat(input);
  const iconName = FILE_ICONS[format] || FILE_ICONS.unknown;

  return `${assetsDir}/icons/files/${iconName}.svg`;
}
